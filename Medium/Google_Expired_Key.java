...
电面那题删除expired key value pair可以用min heap，就是根据expiration的time stamp以及相应的key放到heap里面，每次要清空的时候把heap里面expiration timestamp小于当前时间的都删掉就好了
字典里key就是正常的key，value除了普通的value还包含过期的时间，每次存key，value的时候如果key已经存在就update value和exp time，如果不存在就直接把value和exp time放到字典，每次取value的时候看看现在的时间和exp时间比较，如果过期了就返回null，没过期就返回值
*/
