...
RESULT: 97%
NOTES:
1. 对于有 int 的多项要进行处理，且处理的依据跟 int 大小有关的时候，先看看可不可以排序来简化处理
2. Collections 排序的部分还要学习
...
Sort by start
manage a tmp, 
when overlap, chosing the larger end as the tmp.end
when not, add tmp to result, update the new tmp
after loop, add the last tmp to result
...
class Solution {
    public List<Interval> merge(List<Interval> intervals) {
        ...
        //sorting the intervals with ascending order
        Collections.sort(intervals,new Comparator<Interval>() {
           public int compare(Interval i1, Interval i2) {
                return i1.start - i2.start;       
           } 
...
NOTES: 
METHOD:
顺序遍历 intervals，对于每个 interval 核查剩下的所有 interval 是否和他有相交，若有，则扩大现有的 interval 为新的 interval，继续比较。并将已经 merge 进去的 interval 标记为不用继续比较。
这么做的缺点是可能会漏掉一些在之前未匹配的小 interval，所以最后加入到 result 之前还要和 result 现有的元素进行比较
*/
/**
 * Definition for an interval.
 * public class Interval {
...
class Solution {
    public List<Interval> merge(List<Interval> intervals) {
        ...
        for(int i = 0; i < intervals.size(); i++){
            if(merged.contains(intervals.get(i))) continue;
            Interval tmp = intervals.get(i);

            for(int j = i + 1; j < intervals.size(); j++){
                if(merged.contains(intervals.get(j))) continue;
                tmp = contains(tmp, intervals.get(j), merged);
            }
            int current_size = merged.size();
            for(int k = 0; k < result.size(); k++){
                tmp = contains(tmp, result.get(k), merged);
            }
            result.add(tmp);
            for(int l = current_size; l < merged.size(); l++){
                result.remove(merged.get(l));
            }
            
        }
        return result;
    }
    private Interval contains(Interval a, Interval b, List<Interval> merged){
            
            if(a.start >= b.start && a.start <= b.end || a.end >= b.start && a.end <= b.end || a.start >= b.start && a.end <= b.end){
                int start_v = Math.min(a.start, b.start);
                int end_v = Math.max(a.end, b.end);
                merged.add(a);
                merged.add(b);
                Interval combine = new Interval(start_v,end_v);
                return combine;
            }
            return a;
    }
...
TEST CASE: 
[[1,4],[0,4]]
[[1,3],[8,10],[15,18],[2,6],[6,7]]

[[1,3],[8,10],[15,18],[2,6],[6,7],[0,34],[2,65]] = [[0,65],[8,10],[15,18]] 
[[1,3],[8,10],[15,68],[2,6],[6,7],[0,34],[2,65]] = [[0,65],[8,10],[15,68]] => expected: [[0,68]]
[1,3] 的时候和 [8,10] ,[15,18]还没有交集，到 [2,6],[6,7],[0,34],[2,65] 后面这一串才开始扩大范围
所以第一轮之后 结果为 add [0,65]
还剩下 [8,10],[15,18] 没进入 merged
所以他们会跟这个子集继续查看可不可以 merge，最后俩不能 merge，于是都塞进了 result
