    ...
    public void findSecretWord(String[] wordlist, Master master) {
        ...
        while(sameNum < 6 && tries++ < 10){
            ...
            for(Map.Entry<String, Integer> entry : count.entrySet()){
                if(entry.getValue() < max) word = entry.getKey();
            }
            sameNum = master.guess(word);
...
class Solution {
    public void findSecretWord(String[] wordlist, Master master) {
        ...
        while(sameNum < 6 && tries++ < 10){
            ...
            Pair<String, Integer> pending = new Pair<>("", Integer.MAX_VALUE);
            
            for(String s : wl){
                if(count.get(s) < pending.getValue()){
                    pending = new Pair<>(s, count.get(s));
                }
            }
            String word = pending.getKey();
            sameNum = master.guess(word);
